...
def scal(a, f, batch=False):
    if batch:
        ...
        return ops.tensor_dot(a.reshape(-1), f.reshape(-1), 1)
...
def squared_distances(x, y, use_keops=False):
    ...
    if x.dim() == 2:
        ...
        D_xy = matmul(x, y.permute(1, 0))  # (N,D) @ (D,M) = (N,M)
        D_yy = (y * y).sum(-1).unsqueeze(0)  # (1,M)
    elif x.dim() == 3:  # Batch computation

        # print((x * x))
        # print((x * x).sum(-1))
        # print(expand_dims((x * x).sum(-1), 2))
        # print((x * x).shape)
        # print((x * x).sum(-1).shape)
        # print(expand_dims((x * x).sum(-1), 2).shape)
        # print('-'*40)

        # D_xx = (x * x).sum(-1).unsqueeze(2)  # (B,N,1)

        D_xx = expand_dims((x * x).sum(-1), 2)  # (B,N,1)

        # D_xy = torch.matmul(x, y.permute(0, 2, 1))  # (B,N,D) @ (B,D,M) = (B,N,M)
        input_perm = (0, 2, 1)
        # print(y)
        t_y = transpose(y, input_perm)
        # print(t_y)
        D_xy = matmul(x, t_y)  # (B,N,D) @ (B,D,M) = (B,N,M)

        # D_yy = (y * y).sum(-1).unsqueeze(1)  # (B,1,M)
        D_yy = expand_dims((y * y).sum(-1), 1)
...
def distances(x, y, use_keops=False):
    ...
    if use_keops:
        ...
        # return sqrt(torch.clamp_min(squared_distances(x, y), 1e-8))
        return sqrt(mindspore.ops.clip_by_value(x, 1e-8, 50000000))
...
avg_pool = ops.AvgPool(kernel_size=2)

subsample = {
    2: (lambda x: 4 * avg_pool(x)),
    3: (lambda x: 8 * avg_pool(x)),
}
...
def C_transform(G, tau=1, p=2):
    ...
    x = mindspore.numpy.arange(N).type_as(G)  # [0, ..., N-1], on the same device as G.
    if p == 1:
        x = x / tau
    if p == 2:
        x = x / np.sqrt(2 * tau)
...
def softmin_grid(eps, C_xy, h_y):
    ...
    x = mindspore.numpy.arange(N).type_as(h_y) / N
    p = C_xy
    if p == 1:
        x = x / eps
    elif p == 2:
