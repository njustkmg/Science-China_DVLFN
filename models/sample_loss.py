...
# from models.kernel_samples import kernel_tensorized, kernel_online, kernel_multiscale
# from models.kernel_samples import kernel_tensorized, kernel_online

from models.sinkhorn_samples import sinkhorn_tensorized
# from models.sinkhorn_samples import sinkhorn_online
# from models.sinkhorn_samples import sinkhorn_multiscale

# from models.kernel_samples import kernel_tensorized as hausdorff_tensorized
# from models.kernel_samples import kernel_online as hausdorff_online
# from models.kernel_samples import kernel_multiscale as hausdorff_multiscale
...
class SamplesLoss(nn.Cell):
    ...
    def construct(self, *args):
        ...
        if l_x is not None or l_y is not None:
            if backend in ["auto", "multiscale"]:
                ...
                # raise ValueError(
                   print('Explicit cluster labels are only supported with the "auto" and "multiscale" backends.')
                # )
        ...
        if backend in ["multiscale"]:  # multiscale routines work on single measures
            if B == 1:
                α, x, β, y = self.expand_dims(α, 0), self.expand_dims(x, 0), self.expand_dims(β, 0), self.expand_dims(y, 0)
            elif B > 1:
                warnings.warn(
    ...
    def process_args(self, *args):
        ...
        if len(args) == 4:
            ...
            # raise ValueError(
               print("A SamplesLoss accepts two (x, y), four (α, x, β, y) or six (l_x, α, x, l_y, β, y)  arguments.")
            # )
    ...
    def generate_weights(self, x):
        ...
        if x.dim() == 2:  #
            ...
            # print(x.shape)
            # print(torch.ones(N))
            # print(torch.ones(N).type_as(x))
            # print(torch.ones(N).type_as(x) / N)
            # print(ones(N, x.dtype) / N)
            # print('-'*40)
            return self.ones(N, x.dtype) / N
        elif x.dim() == 3:
            B, N, _ = x.shape
            # ones((B, N), x.dtype) / N
            return self.ones((B, N), x.dtype) / N
        else:
            raise ValueError(
        ...
        if (
            ...
            if l_x is not None:
                if l_x.dim() not in [1, 2]:
                    ...
                    if l_x.shape[1] > 1:
                        # raise ValueError(
                           print("Without batches, the vector of labels 'l_x' should be encoded as (N,) or (N,1) tensors.")
                        # )
                    l_x = l_x.view(-1)
                if len(l_x) != N:
                    # raise ValueError(
                       print( "The vector of labels 'l_x' should have the same length as the point cloud 'x'.")
                    # )
