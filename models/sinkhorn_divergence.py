...
from models.sam_utils import scal
...
class UnbalancedWeight(nn.Cell):
...
def max_diameter(x, y):
    ...
    # print(x)
    # print(ops.ArgMinWithValue()(x))
    # print(stack([ops.ArgMinWithValue()(x)[1], ops.ArgMinWithValue()(y)[1]]))
    # print('-'*40)
    mins = ops.ArgMinWithValue()(stack([ops.ArgMinWithValue()(x)[1], ops.ArgMinWithValue()(y)[1]]))[1]
    maxs = ops.ArgMaxWithValue()(stack([ops.ArgMinWithValue()(x)[1], ops.ArgMinWithValue()(y)[1]]))[1]
    norm = nn.Norm(axis=0)
    # print(maxs - mins)
    # print(mins)
    # print(maxs)
    diameter = norm(maxs - mins)
    # print(diameter)
    # print('-' * 50)
    # print(type(diameter))
    diameter = diameter.asnumpy()
    # print(type(diameter))
...
def sinkhorn_loop(
    ...
    if last_extrapolation:
        ...
        f_ba, g_ab = (
            damping * softmin(eps, C_xy, stop_gradient(b_log + g_ab / eps)),
            damping * softmin(eps, C_yx, stop_gradient(a_log + f_ba / eps)),
        )

        if debias:
            f_aa = damping * softmin(eps, C_xx, stop_gradient(a_log + f_aa / eps))
            g_bb = damping * softmin(eps, C_yy, stop_gradient(b_log + g_bb / eps))
