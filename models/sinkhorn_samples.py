...
from models.sam_utils import scal, squared_distances, distances
...
def softmin_tensorized(eps, C_xy, h_y):
    ...
    """
    B = C_xy.shape[0]
    infos = logsumexp((h_y.view(B, 1, -1) - C_xy / eps).asnumpy(), axis=2)
    infos = mindspore.Tensor(-eps * infos)
    # print(infos)
    # print('-'*40)
    return (infos).view(B, -1)
...
def sinkhorn_tensorized(
    ...
    **kwargs,
    ...
    # grad_op = ops.GradOperation(get_by_list=False)
    # C_xy = cost(x, y.detach())  # (B,N,M) torch Tensor
    # C_yx = cost(y, x.detach())  # (B,M,N) torch Tensor

    # C_xy = cost(x, grad_op(y))  # (B,N,M) torch Tensor
    # C_yx = cost(y, grad_op(x))  # (B,M,N) torch Tensor
    C_xy = cost(x, stop_gradient(y))  # (B,N,M) torch Tensor
    C_yx = cost(y, stop_gradient(x))  # (B,M,N) torch Tensor
    ...
    C_xx = cost(x, stop_gradient(x)) if debias else None  # (B,N,N) torch Tensor
    C_yy = cost(y, stop_gradient(y)) if debias else None  # (B,M,M) torch Tensor
...
#     smin = (h_j - C_ij * mindspore.Tensor([1 / eps]).type_as(x)).logsumexp(2).view(B, -1)
#
#     return -eps * smin
...
def softmin_online(eps, C_xy, h_y, log_conv=None):
    ...
    out = -eps * log_conv(x, y, h, mindspore.Tensor([1 / eps]).type_as(x))
...
#         if B > 1:
#             # raise ValueError(
#                print("Custom cost functions are not yet supported with batches." "")
#             # )
#         expend = ops.ExpandDims()
#
#         x = expend(x, 0)  # (1, N, D) -> (N, D)
#         y = expend(y, 0)  # (1, M, D) -> (M, D)
#
...
def softmin_multiscale(eps, C_xy, f_y, log_conv=None):
    ...
    # KeOps is pretty picky on the input shapes...
    return -eps * log_conv(
        x, y, f_y.view(-1, 1), mindspore.Tensor([1 / eps]).type_as(x), ranges=ranges_xy
    ).view(-1)
...
#         x_labels, perm = sort(x_lab.view(-1))
#         a, x = a[perm], x[perm]
#
